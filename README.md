...
이 프로젝트는 Vercel 배포 안정성을 위해 **npm 기준**으로 설치/빌드합니다. Node.js는 Next.js 16 기준으로 20.9 이상을 권장합니다.
...
`.npmrc`는 npm 공식 registry를 사용하도록 최소 설정만 포함합니다. Vercel Build Settings에서도 Install Command는 `npm install`, Build Command는 `npm run build`를 사용하세요.
...
Vercel → 프로젝트 → Settings → Build and Development Settings에서 아래처럼 설정하세요.
...
6. Root Directory: GitHub 저장소 루트에 `package.json`이 바로 있으면 빈칸, `POPKIT/package.json` 구조면 `POPKIT`
7. 설정 저장 후 Deployments에서 최신 배포를 선택하고 `Redeploy`를 누른 뒤 `Use existing Build Cache`를 끄고 다시 배포하세요.

### CLI 배포

Vercel CLI를 사용할 수 있는 환경에서는 아래처럼 배포할 수 있습니다.
...
현재 Codex 컨테이너에서는 Vercel API/Registry 접근이 proxy `403 Forbidden`으로 차단되어 실제 preview URL 생성까지는 진행할 수 없었습니다. registry/API 접근이 허용된 Vercel 환경에서 위 npm 기준 설정으로 재배포하면 preview/production URL이 생성됩니다.
