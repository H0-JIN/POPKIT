    ...
    <header className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-white/[0.1] via-white/[0.04] to-cyan-300/[0.04] p-6 shadow-2xl shadow-black/20 sm:p-8">
      <div className="grid gap-7 lg:grid-cols-[1fr_260px] lg:items-start">
        ...
        <aside className="rounded-3xl border border-white/10 bg-zinc-950/35 p-4">
          <div className="flex flex-wrap gap-2"><MetaPill label={t.detail.price} value={translatePricing(tool.pricing, locale)} /><MetaPill label={t.detail.difficulty} value={translateDifficulty(tool.difficulty, locale)} /><MetaPill label={t.detail.korean} value={tool.korean_support ? t.detail.supported : t.detail.needsCheck} /></div>
          <div className="mt-5 flex flex-col gap-3"><a href={tool.official_url} target="_blank" rel="noreferrer" className="rounded-2xl bg-cyan-300 px-5 py-3 text-center text-sm font-black text-zinc-950 hover:bg-cyan-200">{t.detail.officialSite}</a><button className="rounded-2xl border border-white/10 px-5 py-3 text-sm font-bold text-zinc-200 hover:bg-white/5">{t.detail.favorite}</button><span className="text-center text-xs text-zinc-500">{getDomain(tool.official_url)}</span></div>
        </aside>
      </div>
    </header>
  );
