    ...
    <Link href={`/tools/${tool.slug}`} className="group flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-white/10 bg-zinc-900/70 shadow-2xl shadow-black/20 transition hover:-translate-y-1 hover:border-cyan-300/40 hover:bg-zinc-900">
      ...
      <div className="flex flex-1 flex-col gap-4 p-5">
        ...
        <div>
          ...
          {tool.editor_quote ? <p className="mt-2 line-clamp-2 border-l-2 border-cyan-300/50 pl-3 text-[15px] font-semibold leading-6 text-cyan-100/90">“{tool.editor_quote}”</p> : null}
          <p className="mt-3 min-h-12 text-sm leading-6 text-zinc-400">{tool.short_description}</p>
        </div>
