...
export function ToolCard({ tool }: { tool: Tool }) {
  return (
    <Link href={`/tools/${tool.slug}`} className="group overflow-hidden rounded-[1.75rem] border border-white/10 bg-zinc-900/70 shadow-2xl shadow-black/20 transition hover:-translate-y-1 hover:border-cyan-300/40 hover:bg-zinc-900">
      ...
      <div className="space-y-4 p-5">
        <div className="flex flex-wrap gap-2">{tool.tags.slice(0, 3).map((tag) => <span key={tag} className="rounded-full bg-white/5 px-2.5 py-1 text-[11px] font-semibold text-zinc-300">{tag}</span>)}</div>
        <div><h3 className="text-xl font-bold text-white group-hover:text-cyan-200">{tool.tool_name}</h3><p className="mt-2 min-h-12 text-sm leading-6 text-zinc-400">{tool.short_description}</p></div>
        <div className="flex flex-wrap gap-2">{tool.recommended_use_cases.slice(0, 3).map((useCase) => <span key={useCase} className="text-xs text-zinc-500">#{useCase}</span>)}</div>
        ...
        <div className="flex items-center justify-between border-t border-white/10 pt-4 text-sm"><span className="text-zinc-500">{getDomain(tool.official_url)}</span><span className="font-semibold text-cyan-200">자세히 보기 →</span></div>
      </div>
    </Link>
  );
}
