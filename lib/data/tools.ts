...
const canonicalToolNames: Record<string, string> = {
  "napkin-ai": "Napkin AI",
  "kling-ai": "Kling AI",
  lovable: "Lovable"
};

const canonicalToolSlugs: Record<string, string> = {
  "napkin-ai": "napkin-ai",
  "kling-ai": "kling-ai",
  lovable: "lovable"
};
...
function hasOfficialUrl(tool: Tool) {
  return Boolean(tool.official_url && tool.official_url !== "#");
}
...
function completionScore(tool: Tool) {
  return [
    ...
    hasOfficialUrl(tool) ? tool.official_url : "",
...
function recordPriority(tool: Tool) {
  return completionScore(tool) + (hasOfficialUrl(tool) ? 100 : 0);
}

function canonicalToolSlug(matchKey: string) {
  return canonicalToolSlugs[matchKey] ?? matchKey.replace(/[^a-z0-9가-힣]+/g, "-").replace(/^-|-$/g, "");
}

function applyCanonicalToolIdentity(tool: Tool, matchKey: string): Tool {
  const canonicalName = canonicalToolNames[matchKey];
  if (!canonicalName) return tool;

  return {
    ...tool,
    tool_name: canonicalName,
    slug: canonicalToolSlug(matchKey)
  };
}

function mergeToolRecords(primary: Tool, secondary: Tool): Tool {
  const base = recordPriority(secondary) > recordPriority(primary) ? secondary : primary;
  ...
  return {
    ...
    official_url: hasOfficialUrl(base) ? base.official_url : fallback.official_url,
    ...zeroRatingState
  };
}
...
function dedupeToolsByName(tools: Tool[]) {
  ...
  tools.forEach((tool) => {
    ...
    const merged = current ? mergeToolRecords(current, tool) : { ...tool, ...zeroRatingState };
    byName.set(matchKey, applyCanonicalToolIdentity(merged, matchKey));
  });
