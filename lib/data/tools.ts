...
const aliases: Record<string, string[]> = {
  ...
  short_description: ["short_description", "description", "설명", "한 줄 설명", "카드 설명"],
  full_description: ["full_description", "상세 설명", "상세 페이지 설명"],
  recommended_use_cases: ["recommended_use_cases", "use_cases", "추천 업무", "추천 사용 업무"],
  recommended_users: ["recommended_users", "추천 사용자", "추천 유저"],
  ...
  youtube_summary: ["youtube_summary", "영상 요약", "사용법 요약", "핵심 요약"],
  usage_steps: ["usage_steps", "사용법", "사용 단계", "기본 사용 흐름", "사용 흐름"],
  rating_average: ["rating_average", "rating", "평점"],
  rating_count: ["rating_count", "평가 수", "평점 수"],
  comment_count: ["comment_count", "댓글 수", "리뷰 수"],
...
function fallbackUsageSteps(name: string, category: string, useTags: string[]) {
  ...
  return [`${name}로 처리할 업무 목표를 먼저 정리합니다.`, "원하는 결과 형식과 톤, 분량을 구체적으로 요청합니다.", "생성 결과를 검토하고 사실 확인이나 후속 수정을 진행합니다."];
}
...
function adapt(row: SheetRow, index: number): Tool | null {
  ...
  const normalizedUsageSteps = usageSteps.length ? usageSteps : seed?.usage_steps?.length ? seed.usage_steps : fallbackUsageSteps(name, category, useTags);
  ...
  const hasCommentCount = Boolean(value(row, "comment_count"));
  ...
  return {
    ...
    comment_count: hasCommentCount ? num(value(row, "comment_count"), 0) : 0,
