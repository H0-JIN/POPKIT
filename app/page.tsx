...
import { buildRolePopularTools } from "@/lib/rolePopularTools";

export default async function Home() {
  const [tools, reviews, views] = await Promise.all([
    getTools(),
    getReviews(),
    getAllToolViews(),
  ]);
  ...
  const rolePopularTools = buildRolePopularTools(viewAwareTools, reviews);

  return (
    <main className="flex min-h-screen">
      <Sidebar />
      <ToolExplorer
        tools={viewAwareTools}
        rolePopularTools={rolePopularTools}
